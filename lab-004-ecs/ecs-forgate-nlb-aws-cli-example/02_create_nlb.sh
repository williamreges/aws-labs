...
if [ ! $ARN_LB ]; then
  ...
  echo " - Criando VPC Link para o Load Balancer ..."
  ARN_LB_CREATED=$(aws elbv2 describe-load-balancers --query "LoadBalancers[?LoadBalancerName=='$LB_NOME'].LoadBalancerArn" --output text --profile $PROFILE)
  aws apigatewayv2 create-vpc-link \
    --name vpc-link-$LB_NOME \
    --target-arns $ARN_LB_CREATED \
    --profile $PROFILE
  echo " ... VPC Link criado com sucesso"
